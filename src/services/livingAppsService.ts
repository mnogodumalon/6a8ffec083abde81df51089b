...
import type { Kursverwaltung, Mitgliederverwaltung, MitgliederKursanmeldung, KursanmeldungFuerInteressenten, CreateKursverwaltung, CreateMitgliederverwaltung, CreateMitgliederKursanmeldung, CreateKursanmeldungFuerInteressenten } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getKursverwaltung(): Promise<Kursverwaltung[]> {
    ...
    return hydrateRecords(records, 'kursverwaltung');
  }
  static async queryKursverwaltung(q: RecordQuery = {}): Promise<Kursverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSVERWALTUNG}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Kursverwaltung[];
  ...
  static async countKursverwaltung(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSVERWALTUNG}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
  ...
  static async getKursverwaltungEntry(id: string): Promise<Kursverwaltung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Kursverwaltung;
    return hydrateRecords([record], 'kursverwaltung')[0];
  }
  static async createKursverwaltungEntry(fields: CreateKursverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.KURSVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'kursverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateKursverwaltungEntry(id: string, fields: Partial<CreateKursverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.KURSVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kursverwaltung') });
    return { ...data, record_id: data.id };
  }
  ...
  static async getMitgliederverwaltung(): Promise<Mitgliederverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDERVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Mitgliederverwaltung[];
    return hydrateRecords(records, 'mitgliederverwaltung');
  }
  static async queryMitgliederverwaltung(q: RecordQuery = {}): Promise<Mitgliederverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDERVERWALTUNG}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Mitgliederverwaltung[];
  ...
  static async countMitgliederverwaltung(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDERVERWALTUNG}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getMitgliederverwaltungEntry(id: string): Promise<Mitgliederverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDERVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Mitgliederverwaltung;
    return hydrateRecords([record], 'mitgliederverwaltung')[0];
  }
  static async createMitgliederverwaltungEntry(fields: CreateMitgliederverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.MITGLIEDERVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'mitgliederverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateMitgliederverwaltungEntry(id: string, fields: Partial<CreateMitgliederverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.MITGLIEDERVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mitgliederverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async deleteMitgliederverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MITGLIEDERVERWALTUNG}/records/${id}`);
  }

  // --- MITGLIEDER_KURSANMELDUNG ---
  static async getMitgliederKursanmeldung(): Promise<MitgliederKursanmeldung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDER_KURSANMELDUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as MitgliederKursanmeldung[];
    return hydrateRecords(records, 'mitglieder_kursanmeldung');
  }
  static async queryMitgliederKursanmeldung(q: RecordQuery = {}): Promise<MitgliederKursanmeldung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDER_KURSANMELDUNG}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as MitgliederKursanmeldung[];
  ...
  static async countMitgliederKursanmeldung(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDER_KURSANMELDUNG}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getMitgliederKursanmeldungEntry(id: string): Promise<MitgliederKursanmeldung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDER_KURSANMELDUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as MitgliederKursanmeldung;
    return hydrateRecords([record], 'mitglieder_kursanmeldung')[0];
  }
  static async createMitgliederKursanmeldungEntry(fields: CreateMitgliederKursanmeldung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.MITGLIEDER_KURSANMELDUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'mitglieder_kursanmeldung') });
    return { ...data, record_id: data.id };
  }
  static async updateMitgliederKursanmeldungEntry(id: string, fields: Partial<CreateMitgliederKursanmeldung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.MITGLIEDER_KURSANMELDUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mitglieder_kursanmeldung') });
    return { ...data, record_id: data.id };
  }
  static async deleteMitgliederKursanmeldungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MITGLIEDER_KURSANMELDUNG}/records/${id}`);
  }

  // --- KURSANMELDUNG_FUER_INTERESSENTEN ---
  static async getKursanmeldungFuerInteressenten(): Promise<KursanmeldungFuerInteressenten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSANMELDUNG_FUER_INTERESSENTEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as KursanmeldungFuerInteressenten[];
    return hydrateRecords(records, 'kursanmeldung_fuer_interessenten');
  }
  static async queryKursanmeldungFuerInteressenten(q: RecordQuery = {}): Promise<KursanmeldungFuerInteressenten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSANMELDUNG_FUER_INTERESSENTEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as KursanmeldungFuerInteressenten[];
  ...
  static async countKursanmeldungFuerInteressenten(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSANMELDUNG_FUER_INTERESSENTEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getKursanmeldungFuerInteressentenEntry(id: string): Promise<KursanmeldungFuerInteressenten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSANMELDUNG_FUER_INTERESSENTEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as KursanmeldungFuerInteressenten;
    return hydrateRecords([record], 'kursanmeldung_fuer_interessenten')[0];
  }
  static async createKursanmeldungFuerInteressentenEntry(fields: CreateKursanmeldungFuerInteressenten): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.KURSANMELDUNG_FUER_INTERESSENTEN}/records`, { fields: cleanFieldsForApi(fields as any, 'kursanmeldung_fuer_interessenten') });
    return { ...data, record_id: data.id };
  }
  static async updateKursanmeldungFuerInteressentenEntry(id: string, fields: Partial<CreateKursanmeldungFuerInteressenten>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.KURSANMELDUNG_FUER_INTERESSENTEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kursanmeldung_fuer_interessenten') });
    return { ...data, record_id: data.id };
  }
  static async deleteKursanmeldungFuerInteressentenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KURSANMELDUNG_FUER_INTERESSENTEN}/records/${id}`);
  }
