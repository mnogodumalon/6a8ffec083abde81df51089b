/**
 * KursverwaltungDialog — pre-generated create/edit dialog for Kursverwaltung.
 *
 ...
 * KursverwaltungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Kursverwaltung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<KursverwaltungDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Kursverwaltung';
...
/** Widened prefill type for KursverwaltungDialog.defaultValues — see file header. */
export type KursverwaltungDialogDefaults = Kursverwaltung['fields'];

interface KursverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Kursverwaltung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: KursverwaltungDialogDefaults;
...
export function KursverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KursverwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<Kursverwaltung['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['kursname', 'kursdatum'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Kursverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'kursverwaltung');
      await onSubmit(clean as Kursverwaltung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Kursverwaltung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('kursverwaltung') })
    : t('new_entity', { entity: appLabel('kursverwaltung') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'kursname': (
      <div key="kursname" className="space-y-1.5">
        <Label htmlFor="kursname">{fieldLabel('kursverwaltung', 'kursname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="kursname"
        ...
        {showErrors && !fields.kursname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kursverwaltung', 'kursname')}</p>
        )}
      </div>
    ),
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        <Label htmlFor="beschreibung">{fieldLabel('kursverwaltung', 'beschreibung')}</Label>
        <Textarea
          id="beschreibung"
          placeholder="Inhalt, Dauer, Voraussetzungen, besondere Hinweise..."
    ...
    'kursdatum': (
      <div key="kursdatum" className="space-y-1.5">
        <Label htmlFor="kursdatum">{fieldLabel('kursverwaltung', 'kursdatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="kursdatum"
        ...
        {showErrors && !fields.kursdatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kursverwaltung', 'kursdatum')}</p>
        )}
      </div>
    ),
    'ort': (
      <div key="ort" className="space-y-1.5">
        <Label htmlFor="ort">{fieldLabel('kursverwaltung', 'ort')}</Label>
        <Input
          id="ort"
          placeholder="z. B. Vereinsheim Saal 1"
          value={fields.ort ?? ''}
    ...
    'max_teilnehmer': (
      <div key="max_teilnehmer" className="space-y-1.5">
        <Label htmlFor="max_teilnehmer">{fieldLabel('kursverwaltung', 'max_teilnehmer')}</Label>
        <Input
          id="max_teilnehmer"
          type="number"
          inputMode="decimal"
  ...
  const FIELD_LABELS: Record<string, string> = {"kursname": "Kursname", "beschreibung": "Beschreibung", "kursdatum": "Kursdatum und Uhrzeit", "ort": "Veranstaltungsort", "max_teilnehmer": "Maximale Teilnehmerzahl"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KURSVERWALTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
